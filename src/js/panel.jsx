...
function Assignee({ assignee, points, error }) {
  let className = "";
  if (points.total >= 11) className = "assignee__points-total--ok";
  if (points.total >= 14) className = "assignee__points-total--too-much";
  if (points.total >= 17) className = "assignee__points-total--way-too-much";
  return (<li className={"assignee" + (error ? " assignee--error" : "")}>
...
function AssigneeDummy() {
  return (<li className="assignees__dummy"><div>Nothing to count</div>The planned sprint<br/>has not yet been
    created<br/>or is still empty</li>)
}

function Error({ isOccurred, text }) {
  return (isOccurred && <div className="error">{text}</div>)
}
    ...
    switch (message?.type) {
      case 'CounterCollectedData':
        ...
        message.data.rows.forEach((row) => {
          if (['Retro AI','Sprint Goal'].includes(row.issue.type)) return;
          ...
          if ((row.points.qa + row.points.fe + row.points.be > 0) && row.points.qa + row.points.fe + row.points.be !== row.points.total) {
            assignees[row.assignee.name].error = true;
